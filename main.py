lista_alunos = []
total_alunos = input('Digite a quantidade de alunos que são inseridos aqui: ')
while not total_alunos.isdigit(): # Para ver se é numero inteiro
    print("Por favor, digite apenas números inteiros!")
    total_alunos = input('Digite a quantidade de alunos que são inseridos aqui: ')
total_alunos = int(total_alunos)
for i in range (total_alunos): # Pega os dados dos alunos
    print(f'\nAluno {i+1}:')
    nome = str(input('Nome:'))
    t1 = float(input('T1: '))
    while t1<0 or t1>10:
